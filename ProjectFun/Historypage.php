...
<div id="modal-wrapper" class="modal">
  ...
  <form class="modal-content animate" method="post" action="login.php?name='.'Historypage.php'.'" onsubmit="return checkLoginForm()">
...
@keyframes zoom {
    ...
    <br>
    <a href="Signout.php?name='.'Historypage.php'.'"><button  style="width:150px; margin-top:0px; margin-left:1080px; height:50px">SignOut
</button></a>
    ...
    ?>
     ...
     <table border="0 solid black" class="t1">
         ...
         <tr><th><a  style="display:block;padding:25px;text-decoration:none;color:#808000
                  ;width:213px;height:13px;font-family:Comic Sans MS;border:0px solid black;padding-top:200px;font-size:24px;" href="#"><center><img src="clock3.png" width="22px" height="22px" />       History        </center></a></th></tr>
...
{
    echo '<center><strong>You must login to view Liked videos</strong></center>';
}
if($_COOKIE['login']=='true')
{
   ...
   if(mysql_connect('localhost','root',''))
   {
       ...
       {
           $query="select video_desc.vid,views_count,video_desc.uid,name,descript,date_up,time_up,image_addr,h_time,h_date from video_desc,history where video_desc.vid=history.vid and video_desc.vid in (select vid from history where uid=$uid) and history.uid=$uid order by hid desc;";
           if($query_res=mysql_query($query))
           {
               ...
               for($i=0;$i<$liked_count;$i++)
               {
                   ...
                   {
                       if($q_mm=mysql_fetch_assoc($q_rr))
                       {
                           $user_name[$i]=$q_mm['u_name'];
                       }
                   }
               }
           }
    ...
    echo "<table border='0 solid black' style='postiton:absolute;margin-top:-1490px;margin-left:350px;'>";
         for($i=0;$i<$liked_count;$i++)
         {
          ...
          $liked_date=$liked_array[$i]['h_date'].' '.$liked_array[$i]['h_time'];
          echo "<tr>
                <td colspan='2' style='padding-top:1em'><img src=$image_src width='330' height='200'/></td>
                <td style='width:500px;padding-top:1em'><a href='Select.php?id=$vid&name=$name' style='text-decoration:none;color:black'><span style='font-family:times new roman; font-weight:bold;font-size:30px'>$name</span><br><span>$user_name[$i]  Views $views</span><br><br><br></a><span>$descript</span><br><span style='font-size:15px'>Uploaded on :$uploaded</span><br><span style='font-size:15px'>Liked on:$liked_date</span></td>
          </tr>";
