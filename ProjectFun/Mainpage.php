    ...
    ?>
     ...
     <table border="0 solid black" class="t1">
         ...
         <tr><th><a  style="display:block;padding:25px;text-decoration:none;color:#808000
                  ;width:213px;height:13px;font-family:Comic Sans MS;border:0px solid black;padding-top:200px;" href="LikedVideos.php"><center><img src="like4.png" width="22px" height="22px" />       Liked Videos        </center></a></th></tr>
...
if(isset($_POST['user_input']))
{
    if(mysql_connect('localhost','root',''))
    {
        ...
        {
            $query_entered=callSpiltFunction($_POST['user_input']);
            $query="SELECT vid,name,descript,image_addr,time_up,date_up FROM video_desc WHERE name LIKE $query_entered OR descript LIKE $query_entered ;";
...
function callSpiltFunction($String)
{
  ...
  foreach($arr as $v)
  {
      if($count_search==$length-1)
      {
          $ret_string=$ret_string."'%$v%'";
      }
      else
      $ret_string=$ret_string."'%$v%' or ";
      $count_search=$count_search+1;
  }
  return $ret_string;
}
echo "<span style='margin-left:350px;font-family:times new roman;font-size:20px'>About $total_result results<span><br><br>";         
echo "<table border='0 solid black' style='postiton:absolute;margin-top:-1490px;margin-left:350px;'>";
         for($i=0;$i<$count;$i++)
         {
          $image_src=str_replace('@','/',$array[$i]['image_addr']);
          $vid=$array[$i]['vid'];
          $name=$array[$i]['name']; 
          $descript=$array[$i]['descript'];
          $uploaded=$array[$i]['date_up'].' '.$array[$i]['time_up']; 
          echo "<tr>
                <td colspan='2'><img src=$image_src width='330' height='200'/></td>
                <td style='width:500px'><a href='Select.php?id=$vid&name=$name' style='text-decoration:none;color:black'><span style='font-family:times new roman; font-weight:bold;font-size:30px'>$name</span><br><br></a><span>$descript</span><br><br><span style='font-weight:bold'>Uploaded on :</span>$uploaded
          </tr>";
